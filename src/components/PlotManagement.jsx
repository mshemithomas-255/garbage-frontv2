...
const PAYMENT_STATUS = {
  ...
  default: { label: "Pending", color: "bg-gray-100 text-gray-800" },
};
...
const PlotManagement = ({ refreshData }) => {
  ...
  const UserRow = ({ user, plot }) => (
    <div className="grid grid-cols-12 gap-3 items-center bg-gray-50 rounded-lg p-3 hover:bg-gray-100 transition">
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex flex-col xs:flex-row gap-2 w-full sm:w-auto">
          <input
            type="text"
            placeholder="Search locations, plots, or users..."
            value={searchTerm}
        ...
        {filteredLocations.map(({ location, plots: locationPlots }) => (
          ...
          >
            ...
            >
              ...
              {!expandedState.locations[location._id] && (
                <div className="px-5 pb-3 flex sm:hidden justify-between text-xs text-gray-300 border-t border-gray-600 pt-2">
                  <span>Plots: {locationPlots.length}</span>
                  <span>
                    Collection:{" "}
                    {location.totalExpectedAmount > 0
                      ? Math.round(
                ...
                {locationPlots.length > 0 ? (
                  locationPlots.map((plot) => (
                    ...
                    >
                      {/* Plot Header */}
                      <div
                        className="px-5 py-3 flex justify-between items-center cursor-pointer hover:bg-gray-100 transition-colors"
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="flex space-x-3 text-xs text-gray-500 mt-0.5">
                              <span>Users: {plot.users?.length || 0}</span>
                              <span>
                                Expected: KSh {plot.expectedAmount || 0}
                              </span>
                      ...
                      {expandedState.plots[plot._id] && (
                        <div className="bg-white px-5 py-3 border-t border-gray-100">
                          <h5 className="text-sm font-semibold text-gray-700 flex items-center mb-3">
                            ...
                            Users in {plot.name} ({plot.users?.length || 0})
                          </h5>

                          {plot.users && plot.users.length > 0 ? (
                            <div>
                              {/* Column Headers */}
                              <div className="grid grid-cols-12 gap-3 mb-2 px-3 py-2 bg-gray-100 rounded-lg text-xs font-semibold text-gray-600">
                  ...
                  <div className="text-center py-8 bg-gray-50">
                    <FaLayerGroup className="text-gray-300 text-3xl mx-auto mb-2" />
